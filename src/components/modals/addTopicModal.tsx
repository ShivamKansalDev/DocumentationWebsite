...
const MenuComponent = () => {
    return(
        <Menu>
        <MenuButton className="border-pink-500 border-2">Choose a type</MenuButton>
        <MenuItems anchor="bottom" className="border-yellow-500 border-2">
            <MenuItem>
                            ...
                            <div className='w-[30%] p-5 h-full bg-gray-300 '>
                                <ul>
                                    {sliderDetails.map((sliderDetailsItem, index) => {
                                        return (
                                            <li onClick={() => {if(sliderRef.current){
                                                sliderRef.current.slickGoTo(index, false)
                                            }}} key={sliderDetailsItem.id} id={sliderDetailsItem.id} className="my-5 mx-2 px-2 cursor-pointer">
                            ...
                            <div className='w-[70%] h-full'>
                                ...
                                <div className="w-full h-full flex flex-col justify-between items-center">
                                    ...
                                    <div className="w-full slider-container ">
                                        <Slider className="h-full border-blue-500 border-2" ref={sliderRef} {...settings}>
                                    ...
                                    <div className="-my-1 w-full flex items-center justify-between">
                                        <span>
                                            <button className={`${currentSlide === 0 ? "hidden" : ""}`} onClick={() => {if(sliderRef.current){sliderRef.current.slickPrev()}}}>
                                                <ChevronLeftIcon height={30} width={30} className="text-gray-600 bg-gray-200 m-3 rounded-full" />
                                            </button>
                                        </span>
                                        <span>
                                            <button className={`${currentSlide === sliderDetails.length - 1 ? "hidden" : ""}`} onClick={() => {if(sliderRef.current){sliderRef.current.slickNext()}}}>
                                                <ChevronRightIcon height={30} width={30} className="text-gray-600 bg-gray-200 m-3 rounded-full" />
                                            </button>
                                        </span>
                                    </div>
                                </div>
